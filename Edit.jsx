...
const Edit = () => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    axios     
      .put(`http://localhost:8080/jobPost/${currId}`, form)
      .then((resp) => {
  ...
  const handleSkillChange = (skillName, isChecked) => {
    if (isChecked) {
      setForm({ ...form, postTechStack: [...form.postTechStack, skillName] });
    } else {
      setForm({ ...form, postTechStack: form.postTechStack.filter(skill => skill !== skillName) });
    }
  };
                    ...
                    <div>
                      <div>
                        <input
                          ...
                          onChange={(e) => handleSkillChange(name, e.target.checked)}
                        />
